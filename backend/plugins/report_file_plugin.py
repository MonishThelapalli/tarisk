...
from config.settings import ensure_storage_path, STORAGE_PATH

# We'll store reports locally under STORAGE_PATH by default. This removes the
# hard dependency on Azure Blob Storage while preserving the same plugin API.
LOCAL_STORAGE_PATH = ensure_storage_path()
...
class ReportFilePlugin:
    ...
    def __init__(self, connection_string, storage_connection_string=None):
        ...
        # storage_connection_string param kept for compatibility but ignored
        self.storage_connection_string = storage_connection_string
        # Use configured storage path (local filesystem)
        self.storage_container = os.getenv("AZURE_STORAGE_CONTAINER", "procurement-expediting-risk-reports")
        self.report_directory = os.getenv("REPORT_STORAGE_PATH", STORAGE_PATH or LOCAL_STORAGE_PATH)
        ...
        # Local storage (no remote blob client)
    ...
    def _upload_to_data_lake(self, filepath: str, filename: str) -> str:
        ...
        try:
            ...
            target_dir = os.path.abspath(self.report_directory)
            ...
            dest_dir = os.path.join(target_dir, year, month)
            ...
            file_url = f"file://{os.path.abspath(dest_path)}"
            print(f"Saved report locally: {file_url}")
            return file_url
