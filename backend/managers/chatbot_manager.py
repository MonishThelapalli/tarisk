...
from config.settings import AI_MODEL_NAME, create_genai_client
...
class ChatbotManager:
    ...
    def __init__(self, connection_string):
        ...
        # Start with minimal initialization for testing
        ...
        # Session management (required for basic operation)
        self.chat_sessions = {}
        ...
        # Map old Bing env var to new SEARCH_PROVIDER/SEARCH_API_KEY variables
        from config import settings as cfg
        self.search_provider = os.getenv("SEARCH_PROVIDER", cfg.SEARCH_PROVIDER)
        self.search_api_key = os.getenv("SEARCH_API_KEY", cfg.SEARCH_API_KEY)
        if self.search_provider.lower() == "bing" and not self.search_api_key:
            print("WARNING: BING_SEARCH_API_KEY not found; please set SEARCH_API_KEY or use SEARCH_PROVIDER=serper")
    ...
    def __del__(self):
        ...
        try:
            ...
            if self.chat_sessions:
                # Ensure we call the correct cleanup method
                try:
                    loop.run_until_complete(self.cleanup_all_sessions())
                except Exception:
                    # Fallback: attempt to call cleanup_all_sessions without awaiting
                    try:
    ...
    async def initialize_session(self, session_id):
        ...
        try:
            # Get configured AI model name and create a GenAI client
            model_name = AI_MODEL_NAME
            client = create_genai_client()

            # Create session with all agents (compatibility wrappers)
            ...
            # If creating the full multi-agent session fails (e.g. our
            # compatibility wrappers don't satisfy semantic_kernel's
            # Agent model), fall back to a simple single-agent session so
            # the service remains available to the UI instead of raising
            # an exception that results in a 500/502.
            print(f"Error in initialize_session for {session_id}: {e}")
            import traceback
            traceback.print_exc()

            try:
                # Attempt to create a minimal fallback session using the
                ...
                assistant_agent = await create_or_reuse_agent(
                    ...
                    plugins=[self.schedule_plugin, self.risk_plugin, LoggingPlugin(self.connection_string)]
                )

                # Minimal SingleAgentChat shim
                class SingleAgentChatFallback:
                    def __init__(self, agent):
                        self.agent = agent

                    async def ask(self, prompt: str, **kwargs):
                        return await self.agent.call(prompt, **kwargs)

                    async def add_chat_message(self, msg):
                        # no-op for compatibility
                        return None

                chat = SingleAgentChatFallback(assistant_agent)

                agents = {ASSISTANT_AGENT: assistant_agent}
                ...
                async with self._session_lock:
                    self.chat_sessions[session_id] = fallback_session

                print(f"Initialized fallback single-agent session for {session_id}")
                return self.chat_sessions[session_id]

            except Exception as fallback_exc:
                print(f"Fallback session initialization also failed: {fallback_exc}")
                import traceback as _tb
                _tb.print_exc()
                # Clean up any partially created session entry
                async with self._session_lock:
                    if session_id in self.chat_sessions:
                        del self.chat_sessions[session_id]
                # Re-raise the original exception so higher layers can handle
                raise
    
    async def _create_agents_for_session(self, client, model_name, session_id, conversation_id):
        """Create or reuse all agents for a session with added political risk JSON plugin.

        Args:
            client: The GenAI client created via create_genai_client()
            model_name: The configured model name to use for agent wrappers
        ...
        """
        # We don't require separate credentials here. 'client' is a genai client
        # passed from initialize_session.
        ...
        # Configure search provider connection information. Prefer SEARCH_PROVIDER
        # and SEARCH_API_KEY environment variables. For backward compatibility
        # also allow BING_SEARCH_API_KEY.
        bing_connection = None
        provider = getattr(self, 'search_provider', os.getenv('SEARCH_PROVIDER', 'serper')).lower()
        api_key = getattr(self, 'search_api_key', os.getenv('SEARCH_API_KEY')) or os.getenv('BING_SEARCH_API_KEY')

        if provider == 'serper' and api_key:
            # We'll store the provider info in a simple dict that other code can
            # use to call the appropriate search API.
            bing_connection = {
                'type': 'serper',
                'api_key': api_key
            }
            print('Configured Serper search provider')
        elif provider in ('bing', 'binggrounding') and api_key:
            bing_connection = {
                'type': 'bing',
                'api_key': api_key
            }
            print('Configured Bing-compatible search provider via API key')
        else:
            print('No search API configured (SEARCH_PROVIDER/SEARCH_API_KEY not set). Search will not be available')
        ...
        # Create chat objects. AgentGroupChat expects objects matching the
        # semantic_kernel Agent model; if our compatibility wrappers don't
        # validate, fall back to a single-agent shim that delegates to the
        # assistant agent. This avoids failing session initialization while
        # we implement a full adapter later.
        # For now use a SingleAgentChat shim unconditionally. The previous
        # attempt to construct an AgentGroupChat was failing because our
        # compatibility wrapper does not yet implement the semantic_kernel
        # Agent interface. Using a single-agent shim keeps sessions working
        # while we implement a full adapter.

        class SingleAgentChat:
            """Minimal chat shim delegating to a single agent's call() method."""
            def __init__(self, agent):
                self.agent = agent

            async def ask(self, prompt: str, **kwargs):
                # Delegate to the agent's async call method
                return await self.agent.call(prompt, **kwargs)

            # Provide attributes other code may check
            @property
            def agents(self):
                return [self.agent]

        assistant_agent = agents.get(ASSISTANT_AGENT)
        chat = SingleAgentChat(assistant_agent)
        parallel_chat = SingleAgentChat(assistant_agent)
    ...
    async def generate_report_directly(self, session, risk_agent_name, latest_responses, conversation_id, session_id, original_message, thread_ids=None):
        ...
        Args:
            ...
            thread_ids: Optional dict of thread id context passed from higher-level orchestration
        ...
        try:
            ...
            try:
                ...
                if reporting_response:
                    ...
                    # Log this action
                    try:
                        # Include thread context in logs when available
                        log_extra = {}
                        if thread_ids:
                            log_extra['thread_ids'] = str(thread_ids)
    ...
    async def process_message(self, session_id, message):
        ...
        Returns:
            dict: The response containing status, message, and conversation_id
        """
        # Quick local-test shortcut: for fast UI testing we support a lightweight
        # live-model fallback that doesn't initialize the full multi-agent
        # orchestration. This avoids the previous fixed-text stub and returns
        # a model-generated reply for the given message when session_id is
        # 'local-test' or 'test-session-1'. This keeps the UX fast while
        # avoiding the complexity of AgentGroupChat during quick tests.
        if session_id in ("local-test", "test-session-1"):
            try:
                client = create_genai_client()
                model = os.getenv('AI_MODEL_NAME') or 'gemini-2.5-pro'
                # Use the models.generate_content API which is available in the
                # installed google.genai client. Provide the user's message as
                # the contents and return the first candidate text.
                if hasattr(client, 'models') and hasattr(client.models, 'generate_content'):
                    ...
                    return {
                        "status": "error",
                        "response": None,
                        "error": "GenAI client does not support models.generate_content",
                ...
                return {
                    "status": "error",
                    "response": None,
                    "error": f"Local test model call failed: {e}",
                    "conversation_id": None
                }
            ...
            try:
                ...
                if query_type["is_specific_risk"]:
                    ...
                    try:
                        ...
                        # If the standard multi-agent flow fails at runtime,
                        # fall back to a single-model GenAI reply so the UI
                        # receives a helpful answer instead of a 500/502.
                        print(f"Standard multi-agent processing failed, falling back to GenAI single-call: {std_err}")
                        import traceback
                        traceback.print_exc()
                        try:
                            client = create_genai_client()
                            ...
                            if hasattr(client, 'models') and hasattr(client.models, 'generate_content'):
                                ...
                                response = {
                                    'status': 'error',
                                    'response': None,
                                    'error': 'GenAI client unavailable for fallback',
                                    'conversation_id': None
                                }
                        except Exception as gen_e:
                            print(f"Fallback GenAI call failed: {gen_e}")
                            response = {
                                'status': 'error',
                                'response': None,
                                'error': f'Fallback generation failed: {gen_e}',
                                'conversation_id': None
                            }
    ...
    async def _store_agent_output(self, session_id, agent_name, agent_output, action="Complete Analysis"):
        """Store agent output in the event log with improved error handling.

        Args:
            session_id (str): Session identifier 
            agent_name (str): Name of the agent
            agent_output (str): Output text from the agent
            action (str, optional): Action label. Defaults to "Complete Analysis"
    ...
    async def force_report_generation(self, session, risk_type, latest_responses, conversation_id, session_id, original_message, thread_ids=None):
...
```"""
    ...
    def cleanup_resources():
        ...
        This function imports Streamlit lazily so the module can be used
        in non-Streamlit contexts (tests, CLI) without requiring Streamlit
        to be installed.
        """
        ...
        if st and "chatbot_manager" in st.session_state:
