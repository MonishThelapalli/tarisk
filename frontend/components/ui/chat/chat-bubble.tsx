...
const ChatBubbleAvatar: React.FC<ChatBubbleAvatarProps> = ({ src, fallback, className }) => (
  <Avatar className={className}>
    {src && String(src).trim() ? <AvatarImage src={src} alt="Avatar" /> : null}
    <AvatarFallback>{fallback}</AvatarFallback>
  </Avatar>
);

// ChatBubbleMessage
const chatBubbleMessageVariants = cva('px-4', {
  variants: {
    variant: {
      received: 'bg-background text-secondary-foreground rounded-r-lg rounded-tl-lg',
      sent: 'py-4 bg-muted text-sm text-default rounded-l-lg rounded-tr-lg',
    },
    layout: {
      default: '',
...
type ChatBubbleActionProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
